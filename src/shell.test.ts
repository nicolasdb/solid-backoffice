...
describe("the tabs", () => {
  it("has Home, one tab per collective, and Pods", () => {
    const tabs = tabsFor([c(1), c(2)], { name: "home" });
    expect(tabs.map((t) => t.label)).toEqual(["Home", "C1", "C2", "Pods"]);
  ...
  it("puts collectives past three under More, and marks More when one of them is open", () => {
    ...
    expect(tabs.map((t) => t.label)).toEqual(["Home", "C1", "C2", "More", "Pods"]);
    expect(tabs.find((t) => t.label === "More")).toMatchObject({ current: true, badge: 2 });
  });

  it("renders Pods as a link, and marks the tab that is open", () => {
    const html = renderShell({ webId: "https://pod.example/me#me", name: "Neil", tabs: tabsFor([], { name: "home" }), body: "" });
    const box = document.createElement("div");
    box.innerHTML = html;
    const places = [...box.querySelectorAll(".tab")].find((t) => t.textContent!.includes("Pods"))!;
